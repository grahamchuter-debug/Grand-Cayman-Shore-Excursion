...
def year_page(year: str, count: int, months: list[str], by_month: dict) -> str:
    tender_note = (
        '<p class="mt-3 text-sm text-amber-900 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3"><strong>Tender port:</strong> Published arrival and departure times are ship itinerary times. Actual hours ashore can be shorter because of tender queues — allow extra return buffer.</p>'
        if year == "2028" else ""
    )
    links = "".join(
